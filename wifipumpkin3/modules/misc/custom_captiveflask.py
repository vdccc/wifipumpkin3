    ...
    def do_install(self, args):
        ...
        if args:
            try:
                ...
                return print(
                    display_messages(
                        "the argument is invalid please type ?install for more information",
                        error=True,
                    )
                )
            ...
            print(
                display_messages("copy content file .zip to {}".format(dest), info=True)
            )
            ...
            print(
                display_messages(
                    "extracted files on : {}".format(temp_path_file_extracted),
                    info=True,
                )
            )
            if not path.isfile(temp_path_file_extracted):
                return print(
                    display_messages(
                        "the file {} not found ".format(temp_path_file_extracted),
                        error=True,
                    )
                )
            ...
            if not path.isdir(temp_templates_path):
                return print(
                    display_messages(
                        "the directory template {} not found ".format(
                            temp_templates_path
                        ),
                        error=True,
                    )
            ...
            print(
                display_messages(
                    "plugin {} install {}".format(
                        plugin_name, setcolor("sucessful", color="green")
                    ),
                    info=True,
                )
            )
            return
        print(display_messages("unknown command: {} ".format(args), error=True))
